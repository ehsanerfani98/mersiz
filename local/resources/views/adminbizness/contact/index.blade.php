        ...
        <div class="col-xs-12 head" style="margin-bottom: 60px;display: flex;justify-content: space-between">
            <div style="min-width: 150px">
                <h2 style="margin-top: 0">
                    <i style="float: right;font-size: 29pt;color: #555;" class="material-icons">perm_contact_calendar</i>
                    <b style="color: #555;margin: 3px 5px 0 0;float: right;font-size: 18pt;">تماس باما</b>
                </h2>
            </div>
            ...
            <div>
                    ...
                    <button onclick="delete_contacts()" type="button" class="btn btn-danger waves-effect" title="حذف دسته جمعی">
                                    ...
                                    <div class="table-responsive">
                                        <table style="width: 100%;" id="posts" class="table4 table-striped table-hover dataTable js-exportable">
                                            ...
                                            @foreach($contacts as $contact)
                                                    <tr id="post{{$contact->id}}">
                                                        <td style="padding: 0;">
                                                            <input name="delete" type="checkbox" id="md_checkbox_{{$contact->id}}" value="{{$contact->id}}" class="filled-in chk-col-light-blue checkBox">
                                                            <label style="margin: 10px 10px 0 0;" for="md_checkbox_{{$contact->id}}"></label>
                                                        </td>
                                                        <td><a>{{$contact->name}}</a></td>
                                                        <td><a>{{$contact->mobile}}</a></td>
                                                        <td><a>{{$contact->email}}</a></td>
                                                        <td>{{$contact->message}}</td>
                                                        <td>{{Verta::instance($contact->created_at)}}</td>
                                                        <td>
                                                                <button onclick="delete_contact(this,'{{$contact->id}}')" type="button" class="btn btn-danger waves-effect" title="حذف" style="padding: 1px 5px;margin-bottom: 5px">
...
@section('script')
    ...
    <script>
            ...
            if ($(ChkBox).is(':checked')){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = '{{route('contact.delete_contacts')}}';
        ...
        function delete_contact(tag,id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var CSRF_TOKEN = '{{ csrf_token() }}';
                    var url = '{{route('contact.delete_contact')}}';
