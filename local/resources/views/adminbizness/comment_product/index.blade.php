                                        ...
                                        <table style="width: 100%;" id="posts" class="table table-striped table-hover dataTable js-exportable">
                                            ...
                                            @foreach($comments as $comment)
                                                    ...
                                                    <tr id="post{{$comment->id}}">
                                                        ...
                                                        <td><a href="/admin/users/{{$comment->user_id}}/edit">{{$comment->user->name.' '.$comment->user->family}}</a></td>
                                                        <td><a href="/product/{{$comment->product->slug}}">{{$comment->product->title}}</a></td>
                                                        <td>{{$comment->title}}</td>
...
@section('script')
    <script>
        ...
        $('.defaultModal').click(function () {
            ...
            var url = '{{route('product.comment-product-details')}}';
            var data = {_token: CSRF_TOKEN, id: id};
            $.post(url, data, function (msg) {
                $('#defaultModalLabel').text(msg.name);
                $('#comment-product-title a').text(msg.product_title);
                $('#comment-product-title a').attr('href','/product/'+msg.product_slug);
                $('#comment-title').text(msg.title);
        ...
        $('#save-modal').click(function () {
            ...
            var CSRF_TOKEN = '{{ csrf_token() }}';
            var url = '{{route('product.comment-product-details-details')}}';
    ...
    <script>
        ...
        function delete_comments() {
            ...
            if ($(ChkBox).is(':checked')){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        var CSRF_TOKEN = '{{ csrf_token() }}';
                        var url = '{{route('comment.delete-comments-product')}}';
        ...
        function delete_comment(tag,id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var CSRF_TOKEN = '{{ csrf_token() }}';
                    var url = '{{route('comment.delete-comment-product')}}';
                    var data = {_token: CSRF_TOKEN, id: id};
                    $.post(url, data, function (msg) {
                        if (msg=="deleted"){
                            $.notify({
