        ...
        <div class="col-xs-12 head" style="margin-bottom: 60px;display: flex;justify-content: space-between">
            <div style="min-width: 150px">
                <h2 style="margin-top: 0">
                    <i style="float: right;font-size: 29pt;color: #555;" class="material-icons">unarchive</i>
                    <b style="color: #555;margin: 3px 5px 0 0;float: right;font-size: 18pt;">درخواست های موجود کردن محصولات</b>
                </h2>
            </div>
            ...
            <div>
                    ...
                    <button onclick="delete_RequestQuantities()" type="button" class="btn btn-danger waves-effect" title="حذف دسته جمعی">
                        ...
                        <div class="body">
                                    ...
                                    <div class="table-responsive">
                                        <table style="width: 100%;" id="posts" class="table4 table-striped table-hover dataTable js-exportable">
                                            ...
                                            <tbody>
                                            @foreach($RequestQuantities as $RequestQuantity)
                                                    <tr id="post{{$RequestQuantity->id}}">
                                                        <td style="padding: 0;">
                                                            <input name="delete" type="checkbox" id="md_checkbox_{{$RequestQuantity->id}}" value="{{$RequestQuantity->id}}" class="filled-in chk-col-light-blue checkBox">
                                                            <label style="margin: 10px 10px 0 0;" for="md_checkbox_{{$RequestQuantity->id}}"></label>
                                                        </td>
                                                        <td><a href="/product/{{$RequestQuantity->product->slug}}">{{$RequestQuantity->product->title}}</a></td>
                                                        <td><a>{{$RequestQuantity->count}}</a></td>
                                                        <td>{{Verta::instance($RequestQuantity->created_at)}}</td>
                                                        <td>
                                                                <button onclick="delete_RequestQuantity(this,'{{$RequestQuantity->id}}')" type="button" class="btn btn-danger waves-effect" title="حذف" style="padding: 1px 5px;margin-bottom: 5px">
...
@section('script')
    ...
    <script>
        ...
        function delete_RequestQuantities() {
            ...
            if ($(ChkBox).is(':checked')){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = '{{route('RequestQuantity.delete_RequestQuantities')}}';
        ...
        function delete_RequestQuantity(tag,id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var CSRF_TOKEN = '{{ csrf_token() }}';
                    var url = '{{route('RequestQuantity.delete_RequestQuantity')}}';
