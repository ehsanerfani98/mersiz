...
@section('Admin_content')
    <div class="col-xs-12 head" style="margin-bottom: 60px;display: flex;justify-content: space-between">
        ...
        <div>
            <a href="{{route('categories.create')}}" type="button" class="btn bg-green waves-effect" title="افزودن جدید">
                            ...
                            <div class="table-responsive">
                                <table style="width: 100%;" id="posts" class="table table-striped table-hover dataTable js-exportable">
                                    ...
                                    @foreach($categories as $category)
                                        @php
                                            $parent = App\Category::where('id', $category->parent)->first();
                                    if(empty($parent)){
                                    $parent = 'ندارد';
                                    }else{
                                    $parent = $parent->title;
                                        ...
                                        <tr id="post{{$category->id}}">
                                            ...
                                            <td>@if($category->imgName!="")<img src="{{asset($category->imgPath)}}" width="100" height="100"> @else بدون تصویر@endif</td>
                                            <td>{{$category->title}}</td>
                                            <td>{{$category->slug}}</td>
                                            <td>{{$parent}}</td>
                                            <td>@if($category->showindex!="NO")نمایش@endif</td>
                                            <td>

                                                <button onclick="delete_product_category(this,'{{$category->id}}')" type="button" class="btn btn-danger waves-effect" style="padding: 1px 5px;margin-bottom: 5px">
                                                    <i class="material-icons" style="font-size: 15px;">delete_forever</i>
                                                    حذف</button>
                                                <a href="{{route('categories.edit',$category->id)}}" type="button" class="btn bg-light-blue waves-effect" style="padding: 1px 5px;margin-bottom: 5px">
