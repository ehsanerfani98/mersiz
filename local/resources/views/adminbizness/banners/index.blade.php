    ...
    <div class="col-xs-12 head" style="margin-bottom: 60px;display: flex;justify-content: space-between">
        <div style="min-width: 150px">
            <h2 style="margin-top: 0">
                ...
                <b style="color: #555;margin: 7px 5px 0 0;float: right;font-size: 18pt;">مدیریت بنر</b>
            </h2>
        </div>
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 slider" style="float: right">
                        <div class="card">
                            <div class="body" style="padding: 0">
                                <div class="image" style="height: 186px">
                                    ...
                                    <img width="100%" src="{{asset($banner->imgPath)}}">
                                </div>
                                <div class="slider-body">
                                    <div class="row clearfix">
                                        <div class="col-sm-12" style="margin-bottom: 0">
                                            ...
                                            <div class="form-group">
                                                <div class="form-line">
                                                    <input type="text" onblur="setText(this,{{$banner->id}})"
                                                           name="link" class="form-control"
                                                           placeholder="توضیحات را وارد کنید" value="{{$banner->text}}">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <div class="form-line">
                                        ...
                                        <div class="form-group">
                                                <div class="form-line">
                                                    <input type="text" onblur="setAlt(this,{{$banner->id}})"
                                                           name="link" class="form-control"
                                                           placeholder="(alt)عنوان تصویر را وارد کنید" value="{{$banner->alt}}">
                                                </div>
                                            </div>
...
<!--
                                            <div class="form-group form-float">
                                                <label>موقعیت</label>
                                                <select required id="position" onchange="change_position(this,{{$banner->id}})"
                                                        name="position"
                                                        class="selectpicker form-control show-tick">
                                                    <option value="" @if($banner->position=="")selected @endif>بدون موقیعت</option>
                                                    <option value="top" @if($banner->position=="top")selected @endif>بنر بالا</option>
&lt;!&ndash;                                                    <option value="center" @if($banner->position=="center")selected @endif>بنر وسط</option>
                                                    <option value="bottom" @if($banner->position=="bottom")selected @endif>بنر پایین</option>&ndash;&gt;
                                                </select>
                                            </div>
-->
                                            ...
                                            <div class="form-group form-float">
                                                <label>وضعیت</label>
                                                <select required id="status" onchange="change_status(this,{{$banner->id}})"
                                                        name="status"
                                                        class="selectpicker form-control show-tick">
                                                    <option value="DontShow" @if($banner->status=="DontShow")selected @endif>نمایش داده نشود</option>
                                                    <option value="Show" @if($banner->status=="Show")selected @endif>نمایش داده شود</option>
                                                </select>
                                            </div>
...
@section('script')
        ...
        function delete_img(tag, id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var url = '{{route('banner.delete_image_banner')}}';
    ...
    <script>
        function setTitle(tag, id) {
            ...
            if (link != "") {
                var CSRF_TOKEN = '{{ csrf_token() }}';
                var url = '{{route('banner.set_Title_banner')}}';
        ...
        function setText(tag, id) {
            ...
            if (link != "") {
                var CSRF_TOKEN = '{{ csrf_token() }}';
                var url = '{{route('banner.set_Text_banner')}}';
                var data = {_token: CSRF_TOKEN, id: id, link: link};
                $.post(url, data, function (msg) {
                    if (msg == "ok") {
                        $.notify({
        ...
        function setLink(tag, id) {
            ...
            if (link != "") {
                var CSRF_TOKEN = '{{ csrf_token() }}';
                var url = '{{route('banner.set_link_banner')}}';
                var data = {_token: CSRF_TOKEN, id: id, link: link};
                $.post(url, data, function (msg) {
                    if (msg == "ok") {
                        $.notify({
        ...
        function setAlt(tag, id) {
            ...
            if (link != "") {
                var CSRF_TOKEN = '{{ csrf_token() }}';
                var url = '{{route('banner.set_alt_banner')}}';
                var data = {_token: CSRF_TOKEN, id: id, link: link};
                $.post(url, data, function (msg) {
                    if (msg == "ok") {
                        $.notify({
        ...
        function change_status(tag, id) {
            var status = $(tag).val();
            var CSRF_TOKEN = '{{ csrf_token() }}';
            var url = '{{route('banner.set_status_banner')}}';
        ...
        function change_position(tag, id) {
            ...
            var CSRF_TOKEN = '{{ csrf_token() }}';
            var url = '{{route('banner.set_position_banner')}}';
