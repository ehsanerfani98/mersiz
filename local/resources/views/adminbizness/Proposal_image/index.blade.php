        ...
        <div class="col-xs-12 head" style="margin-bottom: 60px;display: flex;justify-content: space-between">
            <div style="min-width: 150px">
                <h2 style="margin-top: 0">
                    <i style="float: right;font-size: 29pt;color: #555;" class="material-icons">satellite</i>
                    <b style="color: #555;margin: 3px 5px 0 0;float: right;font-size: 18pt;">مدیریت بنر پایین سایت</b>
                </h2>
            </div>

            <div>
                <a href="{{route('Proposal-image.create')}}" type="button" class="btn bg-green waves-effect" title="افزودن جدید">
                    <i class="material-icons">add_circle</i>
                    <span>افزودن جدید</span>
                </a>

                <button onclick="delete_Proposal_images()" type="button" class="btn btn-danger waves-effect" title="حذف دسته جمعی">
                                ...
                                <div class="table-responsive">
                                    <table style="width: 100%;" id="posts" class="table table-striped table-hover dataTable js-exportable">
                                        ...
                                        @foreach($Proposals as  $Proposal)
                                            <tr id="post{{$Proposal->id}}">
                                                <td style="padding: 0;">
                                                    <input name="delete" type="checkbox" id="md_checkbox_{{$Proposal->id}}" value="{{$Proposal->id}}" class="filled-in chk-col-light-blue checkBox">
                                                    <label style="margin: 10px 10px 0 0;" for="md_checkbox_{{$Proposal->id}}"></label>
                                                </td>
                                                <td style="width: 100px">@if($Proposal->image=="") بدون تصویر @else <img style="width: 100%;max-width: 100px" src="{{asset($Proposal->image)}}">@endif</td>
                                                <td>{{$Proposal->title}}</td>
                                                <td>@if($Proposal->status=="Show")<span style="color: #0f9d58">نمایش</span> @else<span style="color: #f39c12">عدم نمایش</span> @endif</td>
                                                <td>{{Verta::instance($Proposal->created_at)}}</td>
                                                <td>
                                                    <button onclick="delete_Proposal_image(this,'{{$Proposal->id}}')" type="button" class="btn btn-danger waves-effect" title="حذف" style="padding: 1px 5px;margin-bottom: 5px">
                                                        <i class="material-icons" style="font-size: 15px;">delete_forever</i>
                                                        حذف</button>

                                                    <a href="{{route('Proposal-image.edit',$Proposal->id)}}" type="button" class="btn bg-light-blue waves-effect" title="ویرایش" style="padding: 1px 5px;margin-bottom: 5px">
...
@section('script')
    ...
    <script>
        ...
        function delete_Proposal_images() {
            ...
            if ($(ChkBox).is(':checked')){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = '{{route('tool.delete_Proposal_images')}}';
        ...
        function delete_Proposal_image(tag,id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var CSRF_TOKEN = '{{ csrf_token() }}';
                    var url = '{{route('tool.delete_Proposal_image')}}';
