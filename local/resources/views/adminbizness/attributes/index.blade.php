    ...
    <div class="col-xs-12 head" style="margin-bottom: 60px;display: flex;justify-content: space-between">
        <div style="min-width: 150px">
            <h2 style="margin-top: 0">
                <i style="float: right;font-size: 29pt;color: #555;" class="material-icons">kitchen</i>
                <b style="color: #555;margin: 3px 5px 0 0;float: right;font-size: 18pt;">لیست ویژگی ها </b>
            </h2>
        </div>

        <div>
            <a href="{{route('attribute.create')}}" type="button" class="btn bg-green waves-effect" title="افزودن جدید">
                <i class="material-icons">add_circle</i>
                <span>افزودن جدید</span>
            </a>
            <button onclick="delete_attributes()" type="button" class="btn btn-danger waves-effect" title="حذف دسته جمعی">
                            ...
                            <div class="table-responsive">
                                <table style="width: 100%;" id="posts" class="table table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tr>
                                        ...
                                        <th>ویژگی</th>
                                        <th style="width: 180px">مقدار</th>
                                        <td style="width: 180px;text-align: center"> دسته بندی</td>
                                        <th>فعالیت ها</th>
                                    </tr>
                                    </thead>
                                    ...
                                    @foreach($attributes as $attribute)
                                        ...
                                        <tr id="post{{$attribute->id}}">
                                            <td style="padding: 0;">
                                                <input name="delete" type="checkbox" id="md_checkbox_{{$attribute->id}}" value="{{$attribute->id}}" class="filled-in chk-col-light-blue checkBox">
                                                <label style="margin: 10px 10px 0 0;" for="md_checkbox_{{$attribute->id}}"></label>
                                            </td>
                                            <td>{{$attribute->title}}</td>
                                            <td style="width: 180px">
                                                @foreach ($attribute->attribute_values as $attr)
                                                    <span style="margin-bottom: 1px;display: inline-block" class="label label-primary">{{$attr->value}}</span>
                                                @endforeach
                                            </td>
                                            <td style="width: 180px">
                                                @foreach($attribute->categories as $category)
                                                    <span style="margin-bottom: 1px;display: inline-block" class="label label-primary">{{@$category->title}}</span>
                                                @endforeach
                                            </td>
                                            <td>

                                                <button onclick="delete_attribute(this,'{{$attribute->id}}')" type="button" class="btn btn-danger waves-effect" style="padding: 1px 5px;margin-bottom: 5px">
                                                    <i class="material-icons" style="font-size: 15px;">delete_forever</i>
                                                    حذف</button>
                                                <a href="{{route('attribute.edit',$attribute->id)}}" type="button" class="btn bg-light-blue waves-effect" style="padding: 1px 5px;margin-bottom: 5px">
...
@section('script')
    <script>
        ...
        $.notify({
            // options
            message: '<i style="float: right;margin-top: -3px;margin-left: 10px" class="material-icons">warning</i> <span style="float: right"> {{session('attribute')}}</span>',
            icon: '',
        }, {
            // settings
            type: 'success',
    ...
    <script>
            ...
            if ($(ChkBox).is(':checked')){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = '{{route('product.delete_attributes')}}';
        ...
        function delete_attribute(tag,id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var CSRF_TOKEN = '{{ csrf_token() }}';
                    var url = '{{route('product.delete_attribute')}}';
